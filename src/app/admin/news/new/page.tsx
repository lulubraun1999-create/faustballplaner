...
export default function NewArticlePage() {
  ...
  const onSubmit = (data: ArticleFormData) => {
    if (!user) {
      toast({ variant: 'destructive', title: 'Nicht angemeldet', description: 'Sie müssen angemeldet sein, um einen Artikel zu erstellen.' });
      return;
    }
    ...
    addDoc(articlesRef, articleData)
      .then(() => {
        toast({ title: 'Erfolg', description: 'Artikel wurde erfolgreich veröffentlicht.' });
        ...
        <div className="container mx-auto px-4 py-8 md:py-12">
          <Card className="max-w-4xl mx-auto">
            <CardHeader>
              ...
              <CardDescription>
                Fülle die folgenden Felder aus, um einen neuen News-Beitrag zu erstellen.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button type="submit" disabled={isPending} className="bg-destructive hover:bg-destructive/90 text-destructive-foreground">
                    {isPending ? <Loader2 className="animate-spin" /> : 'Artikel veröffentlichen'}
                  </Button>
                </div>
              </form>
            </CardContent>
